...
## Allegra's quick tips

**Shortcuts:** 
'ctrl+alt+I to insert a new chunk | 
alt+enter to run ALL(?) chunks| 
alt+shift+k for overlay showing lots of other shortcuts |
alt+ [minus] for <- within a chunk of R
...
my_summary
```
Onward to more complicated things...
...
class(my_summary)
class(summary)
my_new_vector <- c(my_numbers, "Mozart")
my_new_vector
class(my_new_vector)
```
...
```{r}
## Note this code would not knit or run as specified in data viz:
## Returned message "Error: unexpected '/' in "url <- (https:/"
